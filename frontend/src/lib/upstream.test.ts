...
describe('splitUpstreams', () => {
  it('separa o retry que o Nginx registra na mesma linha', () => {
    expect(splitUpstreams('100.100.0.2:80, 100.100.0.1:80')).toEqual([
      '100.100.0.2:80',
      '100.100.0.1:80',
    ]);
  });
...
describe('upstreamHost', () => {
  it('remove a porta', () => {
    expect(upstreamHost('100.100.0.1:80')).toBe('100.100.0.1');
  });
...
describe('deriveUpstreams', () => {
  ...
  it('mostra o tráfego mesmo quando o .env não bate com os endereços reais', () => {
    const stats = [
      stat('100.100.0.2:80', 9),
      stat('100.100.0.1:80', 8),
    ];
    const ipsDesatualizadosNoEnv = ['198.51.100.25', '198.51.100.39'];
  ...
  // mapeamento não segue o último octeto do IP público — 100.100.0.1 é a
  // VPS-1 (198.51.100.25), não a VPS-2.
  it('mantém Node 1 = VPS-1 com os endereços Tailscale reais', () => {
    const nodes = deriveUpstreams(
      [stat('100.100.0.2:80', 20), stat('100.100.0.1:80', 3)],
      ['100.100.0.1', '100.100.0.2'],
    );

    expect(nodes.map((n) => n.addr)).toEqual(['100.100.0.1:80', '100.100.0.2:80']);
  });
